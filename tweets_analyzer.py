...
class TweetStreamAnalyzer:
    ...
    def train_model(self):
        ...
        stage_3 = Word2Vec(
            inputCol="filtered_words", outputCol="vector", vectorSize=100
        )
    ...
    def run(self):
        ...
        tweets_table = kafka_df.withColumn(
            "message", from_json(col("message"), schema)
        ...
        tweets_table = tweets_table.withColumn(
            "created_at", date_process(tweets_table["created_at"])
        )
        tweets_table = tweets_table.withColumn(
            "created_at",
            unix_timestamp("created_at", "yyyy-MM-dd HH:mm:ss").cast(TimestampType()),
        )

        # remove weblink and special characters
        pre_process = udf(
            lambda x: re.sub(
                r"[^A-Za-z\n ]|(http\S+)|(www.\S+)", "", x.lower().strip()
            ),
            StringType(),
        )
        tweets_table = tweets_table.withColumn(
            "cleaned_data", pre_process(tweets_table.tweet)
        ).dropna()
        prediction = self.pipelineFit.transform(tweets_table).select(
        ...
        prediction = prediction.withColumn(
            "probability", extract_prob_udf(col("probability"))
        )

        # window operation: calculate the average probability in 1 minute window
        df = prediction.groupBy(window(prediction.created_at, "1 minutes")).avg(
            "probability"
        )
        ...
        query = (
            df.selectExpr(
                "CAST(window AS STRING) as key", "CAST(sentiment AS STRING) as value"
            )
